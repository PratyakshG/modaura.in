...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const shipment = await Shipment.findById(id).populate("orderId", "email amount address");
    if (!shipment) {
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const shipment = await Shipment.findByIdAndUpdate(id, validatedData, { new: true, runValidators: true });

    if (!shipment) {
      return NextResponse.json({ error: "Shipment not found" }, { status: 404 });
    }

    return NextResponse.json({ shipment, message: "Shipment updated successfully" }, { status: 200 });
  } catch (error) {
